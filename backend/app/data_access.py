...
async def list_events_for_calendar(
    ...
    """Windowed events plus recurring masters needed for expansion."""
    if _use_supabase_rest():
        return await supabase_store.list_events_for_calendar(
        ...
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                ...
                WHERE user_id = $1
                  ...
                  AND (
                    (date >= $2 AND date <= $3)
                    OR (recurrence_rule IS NOT NULL AND date <= $3)
                  )
                ORDER BY date, start_time
                """,
                user_id,
